...
public class AirportServiceImpl implements AirportService {
	...
	{
		Optional<Airport> findByCode = airportRepository.findByAirportCode(airportCode);
		if (findByCode.isPresent()) {
			return findByCode.get();
		}
		else
		{
			throw new AirportIdNotFoundException("Airport not found with airport code: " + airportCode);
		}
	}
    ...
    {
        Optional<Airport> viewAirportByCode=airportRepository.findByAirportCode(newAirport.getAirportCode());
